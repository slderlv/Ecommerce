...
public class SQLClientService implements ISQLCreate<User>, ISQLDelete<User>, ISQLRead<User>, ISQLUpdate<Client>{
    ...
    public void create(User t) {
        // Register
        try{  
            PreparedStatement statement = SQLConnection.getSQLConnection().connect().prepareStatement("INSERT INTO users(rut,admin,name,password,phone_number,mail,img_path,street,blocked) VALUES (?,false,'',?,0,?,'','',false)");
    ...
    public boolean isBlocked(String t) {
    	try{  
            PreparedStatement statement = SQLConnection.getSQLConnection().connect().prepareStatement("SELECT * FROM users WHERE rut=? AND blocked = true");
            statement.setString(1,t);
            ResultSet response = statement.executeQuery();
